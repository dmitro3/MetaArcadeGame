        ...
        if (pv.IsMine)
        {
            ...
            if (!MetaManager.isPlayingMinigame)
            {
                ...
                if(!EventSystem.current.IsPointerOverGameObject() && playerActionAsset.Player.Attack.triggered && isOnGround && !animator.GetBool("punch") && !animator.GetBool("hit"))
                {
                    ...
                    animator.SetFloat("attack_id", (int)UnityEngine.Random.Range(0,3));
...
/*
    ...
    {
        MetaManager.insta.myPlayer.GetComponent<PlayerController>().GotAttack(this.transform.position, triggerPoint);        
    }
